...
class ComposePreferencesTest(unittest.TestCase):
    """
    Test that various preferences exist and have their settings honored.

    This should maybe use the trial API for creating test cases
    automatically, since they're all basically the same.
    """

    def setUp(self):
        """
        Initialize a store with a ComposePreferenceCollection on it,
        for later testing of its behavior.
        """
        st = store.Store()
        self.prefs = compose.ComposePreferenceCollection(
            store=st,
    ...
    def test_setSmarthost(self):
        """
        Test that setPreferenceValue can set the preferred smarthost.
        """
        self.prefs.setPreferenceValue(
            compose._SmarthostPreference(u'', self.prefs), u'example.net')
        self.assertEquals(self.prefs.preferredSmarthost, u'example.net')


    def test_setSmarthostUsername(self):
        """
        Test that setPreferenceValue can set the smarthost username.
        """
        self.prefs.setPreferenceValue(
            compose._SmarthostUsernamePreference(u'', self.prefs), 
            u'radix')
        self.assertEquals(self.prefs.smarthostUsername, u'radix')


    def test_setSmarthostPassword(self):
        """
        Test that setPreferenceValue can set the smarthost password.
        """
        self.prefs.setPreferenceValue(
            compose._SmarthostPasswordPreference(u'', self.prefs), 
            u'osecret')
        self.assertEquals(self.prefs.smarthostPassword, u'osecret')


    def test_setSmarthostPort(self):
        """
        Test that setPreferenceValue can set the smarthost port.
        """
        self.prefs.setPreferenceValue(
            compose._SmarthostPortPreference(u'25', self.prefs),
            u'28')
        self.assertEquals(self.prefs.smarthostPort, 28)

    def test_setSmarthostAddress(self):
        """
        Test that setPreferenceValue can set the smarthost address.
        """
        self.prefs.setPreferenceValue(
            compose._SmarthostAddressPreference(u'', self.prefs),
            u'radix@example.net')
        self.assertEquals(self.prefs.smarthostAddress, u'radix@example.net')
